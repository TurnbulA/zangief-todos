...
const sortAlphabetical = buttonClassName => {
  sortItem = document.querySelector(buttonClassName);
  const alphabetical = Array.from(document.querySelectorAll(".c-list-item"))
    .map(item => item.innerText)
    .sort();
  if (sortItem.classList.contains("c-menu-control__dropdown--alphabetical")) {
    alphabetical.forEach((item, index) => {
      Array.from(document.querySelectorAll(".c-list-item__text p"))[
        index
      ].innerText = item;
    });
  } else if (
    sortItem.classList.contains("c-menu-control__dropdown--reverseAlpha")
  ) {
    const reverseAlpha = alphabetical.reverse();
    reverseAlpha.forEach((item, index) => {
      Array.from(document.querySelectorAll(".c-list-item__text p"))[
        index
      ].innerText = item;
    });
